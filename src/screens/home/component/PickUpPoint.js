...
import {CrossBlack, GpsIcon, Location, Search, Seprator} from '@/assets';
...
const PickUpPoint = ({
  ...
  const getAddress = (data, details) => {
    ...
    //     setPickupCountry(addressDetails?.[i]?.long_name);
    //   }

    //   if (addressDetails[i].types[0] == 'postal_code') {
    //     setPickupZipCode(addressDetails?.[i]?.long_name);
    //   }

    //   if (addressDetails[i].types[0] == 'administrative_area_level_1') {
    //     setPickupState(addressDetails?.[i]?.long_name);
    //   }
    ...
    //   ) {
    //     setPickupCity(addressDetails?.[i]?.long_name);
    //   }
    // }
  };
      ...
      <View
        ...
        <GooglePlacesAutocomplete
          ...
          onPress={(data, details) => {
            setPickupStreetAddress(data?.description);
